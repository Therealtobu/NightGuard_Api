...
"""NightGuard V2 - VM Runtime Generator (hardened, numeric dispatch, minified)"""
...
local function _N_decrypt(bc,k32,sd)local kl=#k32;local t={};for i=1,#bc do t[i]=_bxor(bc[i],k32[((i-1)%kl)+1])end;local o={};local k=sd;for i=1,#t do local e=t[i];o[i]=_bxor(e,k);k=(k*13+e)%256;if k==0 then k=1 end end;return o end
local function _N_reader(bytes)local pos=1;local R={}
...
local function _N_load_proto(R)local p={};p.nparams=R.u8();p.is_vararg=R.u8()==1;local nc=R.u32();p.code={};for i=1,nc do p.code[i]=R.u32()end;local nk=R.u32();p.consts={};for i=1,nk do local t=R.u8();if t==0 then p.consts[i]=nil elseif t==1 then p.consts[i]=R.u8()~=0 elseif t==2 then p.consts[i]=R.f64()elseif t==3 then p.consts[i]=R.str()elseif t==4 then local sd=R.u8();local st=R.u8();local len=R.u32();local enc={};for j=1,len do enc[j]=R.u8()end;p.consts[i]=_N_dec_str(enc,sd,st)end end;local np=R.u32();p.protos={};for i=1,np do local blk=R.blk();p.protos[i]=_N_load_proto(_N_reader(blk))end;return p end
local function _unpack(instr)local op=math.floor(instr/16777216)%256;local a=math.floor(instr/4096)%4096;local b=instr%4096;return op,a,b end
...
PUSH(function(...)local a={...};local L={};for i=1,p2.nparams do L[i]=a[i]end;local va={};if p2.is_vararg then for i=p2.nparams+1,#a do va[#va+1]=a[i]end end;return _N_exec(p2,uenv,va)end)
...
_N_vm=function(bc,key,seed)local dec=_N_decrypt(bc,key,seed);local R=_N_reader(dec);local proto=_N_load_proto(R);local env=getfenv and getfenv(0)or _G;_N_exec(proto,env,{})end
end
'''

# Canonical name -> numeric ID used in VM dispatch
_CANON_ID = {
    ...
    'DIV':           24, 'MOD':           25, 'POW':           26,
    'CONCAT':        27, 'UNM':           28, 'NOT':           29,
    'LEN':           30, 'EQ':            31, 'NEQ':           32,
    'LT':            33, 'LE':            34, 'GT':            35,
    'GE':            36, 'MAKE_CLOSURE':  37,
    'DUP':           38, 'SWAP':          39,
    'VARARG':        40, 'SELF':          41,
    # fake/junk ops map to 0 (no-op in VM - not listed in dispatch)
    'JUNK': 0, 'FAKE_STACK': 0, 'FAKE_MATH': 0,
}

def generate_vm(opcodes) -> str:
    """
    Build _N_op table: alias_val -> numeric_id
    VM dispatches on numeric id, not string name.
    """
    lines = ['local _N_op={}']
