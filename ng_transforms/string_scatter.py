...
def scatter_string_table(string_table:dict,rng)->dict:
    ...
    for idx,entry in string_table.items():
        ...
        if n<4:
            ...
            continue
        # Split into 2-4 real chunks (random cuts, reassembled in order)
        ...
        # Shuffle chunk order + keep mapping so VM knows reassembly order
        order=list(range(len(chunks)))
        rng.shuffle(order)
        chunks_shuffled=[chunks[i] for i in order]
        # Noise chunks: random offsets, lengths — never referenced by VM
        noise_n=rng.randint(1,3)
        noise=[(rng.randint(0,max(0,n-2)),rng.randint(1,max(1,n//4))) for _ in range(noise_n)]
        # Store: enc_bytes, seed, step, sub_key, real_chunks(shuffled+order), noise
        new[idx]=(enc_bytes,seed,step,sub_key,chunks_shuffled,noise,order)
    return new
